package com.polly.student.adapter
...
import com.polly.student.data.Student
import com.polly.student.databinding.StudentListItemBinding

/**
 * RecyclerView 에 데이터를 보여주기 위한 helper 클래스
 *
 * 클래스를 상속받았을 때 강제로 member 들을 implements 해야 하는 경우가 있다.
 * 이것은 원본 클래스가 
 * 인터페이스 이거나 추상클래스 일 경우이다.
 *
 * 클래스를 상속받았는데 오류가 발생하고
 * implements member 메시지가 나오면 method 를 모두 추가하여 코드를 작성해주어야 한다.
 */
class StudentRecyclerAdapter(private val studentList : LiveData<List<Student>>) // 화면에 보여줄 데이터 받기
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val studentHolder = StudentListItemBinding.inflate(
    ...
    inner class StudentViewHolder(private val binding: StudentListItemBinding)
        : RecyclerView.ViewHolder(binding.root) {
            fun bind(student: Student) {
                 ...
                 *
                 * cf)
                 * data binding 을 이용하면 값의 주소가 복사되므로
                 * binding 에 값을 재할당할 경우 원본 객체의 데이터도 변경된다.
                 */
